...
app.patch("/user/:id",async (req,res)=>{
  try {
    const usa=await User.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    return res.status(201).send(usa);
  } catch (e) {
...
app.delete("/user/:id",async (req,res)=>{
  try {
    const usa=await User.findByIdAndDelete(req.params.id);
    return res.status(201).send(usa);
  } catch (e) {
    return res.status(500).json({message: e.message, status: "Failed"});
  }
